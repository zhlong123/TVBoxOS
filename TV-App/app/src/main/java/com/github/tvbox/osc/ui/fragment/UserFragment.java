...
public class UserFragment extends BaseLazyFragment implements View.OnClickListener {
    ...
    /** 离开主页时重置豆瓣热播区焦点态，避免高 Z 卡片叠到 ViewPager 其它分类页。 */
    public static void clearHotListOverlay() {
        if (tvHotList == null) {
            return;
        }
        for (int i = 0; i < tvHotList.getChildCount(); i++) {
            FocusAnimHelper.resetPosterItem(tvHotList.getChildAt(i));
        }
    }
