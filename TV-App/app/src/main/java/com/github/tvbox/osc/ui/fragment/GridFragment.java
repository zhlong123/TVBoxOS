...
import com.github.tvbox.osc.util.VodTagHelper;
...
public class GridFragment extends BaseLazyFragment {
        ...
        gridAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                if (video != null) {
                    ...
                    if (VodTagHelper.isCategoryNavigationEntry(video)) {
                        focusedView = view;
                        boolean isFolder = video.tag != null && "folder".equalsIgnoreCase(video.tag);
                        if (isFolder && ("12".indexOf(getUITag()) != -1)) {
                            changeView(video.id, true);
                        } else {
        ...
        sourceViewModel.listResult.observe(this, new Observer<AbsXml>() {
            ...
            public void onChanged(AbsXml absXml) {
                ...
                if (absXml != null && absXml.movie != null && absXml.movie.videoList != null && absXml.movie.videoList.size() > 0) {
                    List<Movie.Video> videos = excludeCategoryNavigationEntries(absXml.movie.videoList);
                    if (videos.isEmpty()) {
                        if (page == 1) {
                    ...
                    if (page == 1) {
                        showSuccess();
                        isLoad = true;
                        hasActionItems = hasActionVideo(videos);
                        gridAdapter.setNewData(videos);
                    } else {
                        hasActionItems = hasActionItems || hasActionVideo(videos);
    ...
    /** 网格模式不展示 folder/cover 导航项，与 VodTagHelper 及点击 changeView 逻辑一致。 */
    private List<Movie.Video> excludeCategoryNavigationEntries(List<Movie.Video> videos) {
        if (videos == null || videos.isEmpty() || isFolederMode()) {
            return videos;
        }
        return VodTagHelper.excludeCategoryNavigationEntries(videos);
    }
    ...
    public boolean isFilterShowing() {
        return gridFilterDialog != null && gridFilterDialog.isShowing();
    }

    /** 关闭子分类筛选并聚焦影片卡片。 */
    public boolean dismissFilterIfShowing() {
        if (!isFilterShowing()) {
            return false;
        }
        GridFilterDialog dialog = gridFilterDialog;
        // 必须在按键/焦点导航回调之外 dismiss，否则 ViewRootImpl 仍处理 UP 时会 NPE
        if (mGridView != null) {
            mGridView.post(() -> {
    ...
    public void showFilter() {
        ...
        if (isFilterShowing()) {
            dismissFilterIfShowing();
            return;
        }
        if (gridFilterDialog == null) {
            gridFilterDialog = new GridFilterDialog(mContext);
            gridFilterDialog.setDismissListener(this::focusGridAfterFilter);
            setFilterDialogData();
        }
        gridFilterDialog.show();
    }

    private void focusGridAfterFilter() {
        if (mGridView == null || !isAdded()) {
            return;
        }
        mGridView.postDelayed(() -> {
            if (!isAdded() || mGridView == null) {
                return;
            }
            mGridView.scrollToPosition(0);
            mGridView.setSelection(0);
            mGridView.post(() -> {
                if (!isAdded() || mGridView == null || mGridView.getLayoutManager() == null) {
                    return;
                }
                View firstItem = mGridView.getLayoutManager().findViewByPosition(0);
                if (firstItem != null) {
                    View focusTarget = firstItem.findViewById(R.id.mItemFrame);
